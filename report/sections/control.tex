...
The FIR reference makes the assumption that the maximum pitch angle and maximum fan force can be achieved instantaneously. Therefore, it is not precisely feasible, but it is a smooth reference and can be computed very quickly.
...
\label{eq:opt}
\]
The final time $t_f$ is left free, as it was found that the solution transitions within a good margin of the specification. The above formulation is for the outward trajectory; for the way back, the boundary constraints on $\Theta$ are reversed. 
